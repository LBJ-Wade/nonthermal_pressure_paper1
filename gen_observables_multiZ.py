...
multimah_root = home_dir / 'scratch60/frank_mah/output/redshifts'
...
def multimah_multiM(z_obs, cosmo, Nmah):
    ...
    mah_dir = multimah_root / ('z%03d' % (int(100*z_obs)))
...
def gen_obs(cosmo, beta=beta_def, eta=eta_def):
    ...
    rads = np.logspace(np.log10(0.01),np.log10(N_r200m_mult), Nradii) # y_SZ goes out to 20x R_500c/ 8x R_200m for LOS integration
...
np.savez('z%03d_data.npz' % int(100*zobs), data=data, cvirs=cvirs, Rvirs=Rvirs)
