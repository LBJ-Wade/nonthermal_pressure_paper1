      ...
      if((fl.gt.0..and.fh.lt.0.).or.(fl.lt.0..and.fh.gt.0.))then
        ...
        do 11 j=1,MAXIT
          ...
          if(sign(fm,fnew).ne.fm) then
            ...
            CALL Terminate('never get here in zriddr2')
          endif
          if(abs(xh-xl).le.xacc) return
11      continue
        CALL Terminate('zriddr2 exceed maximum iterations')
      else if (fl.eq.0.) then
        zriddr2=x1
      else if (fh.eq.0.) then
        zriddr2=x2
      else
        CALL Terminate('root must be bracketed in zriddr2')
      endif
      return
      END
...
c---
      ...
      REAL f1,f2
      LOGICAL succes
      if(x1.eq.x2) CALL Terminate('you need initial range in zbrac')
      f1=func(x1)
      f2=func(x2)
      succes=.true.
      do 11 j=1,NTRY
...
CU    USES polint
      ...
      do 11 j=1,JMAX
        ...
        h(j+1)=h(j)/9.
11    continue
      CALL Terminate('too many steps in qromo')
      END
...
CU    USES polint
      ...
      do 11 j=1,JMAX
        ...
        h(j+1)=h(j)/9.
11    continue
      CALL Terminate('too many steps in qrompnt')
      END
...
CU    USES polint3
      ...
      do 11 j=1,JMAX
        ...
        h(j+1)=h(j)/9.
11    continue
      CALL Terminate('too many steps in qromo3')
      END
...
11    continue
      ...
      do 13 m=1,n-1
        do 12 i=1,n-m
          ...
          w=c(i+1)-d(i)
          den=ho-hp
          if(den.eq.0.) CALL Terminate('failure in polint3')
          den=w/den
          d(i)=hp*den
          c(i)=ho*den
12      continue
...
c---
     ...
     *z).gt.BIG) CALL Terminate('invalid arguments in ellrf')
      xt=x
      yt=y
      zt=z
1     continue
...
c---
     ...
     *z).gt.BIG) CALL Terminate('invalid arguments in ellrd')
      xt=x
      yt=y
      zt=z
      sum=0.
...
5       arr(l)=arr(j)
        arr(j)=a
        jstack=jstack+2
        if(jstack.gt.NSTACK) CALL Terminate('NSTACK too small in sort')
...
5       arr(l)=arr(j)
        ...
        brr(j)=b
        jstack=jstack+2
        if(jstack.gt.NSTACK) CALL Terminate('NSTACK too small in sort2')
        if(ir-i+1.ge.j-l)then
          istack(jstack)=ir
          istack(jstack-1)=i
          ir=j-1
...
c---
      ...
      INTEGER j
      REAL p,s,ep
      if(n.le.1) CALL Terminate('n must be at least 2 in moment')
      s=0.
      do 11 j=1,n
        s=s+data(j)
11    continue
